...
5. Remove fields with an `@external` directive _unless they are part of the `@key`_. Stitching expects schemas to only publish fields that they have data for. Remove any remaining `@external` directives.
6. Remove all `@provides` directives. They are no longer necessary once the indirection of `@external` fields is eliminated. Stitching's query planner can automate the optimal selection of as many fields as possible from as few services as possible.

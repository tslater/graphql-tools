...
Well, just creating a new document for our avatar won’t solve the reusability issue.

**2. Use variables and rely on the query to have those defined**

You can already use variables inside fragments. We just need to ensure that the query that uses the fragment also has those variables in the variable definition.
...
It is impossible to use a different width and height for the second usage of the fragment in that query.

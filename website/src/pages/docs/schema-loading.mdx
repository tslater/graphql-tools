...
The user is given the option of implementing their loader (implement the interface `SchemaLoader`).
...
<Callout type="info">`loadFiles` doesn't support `#import` syntax. See below if you need that.</Callout>
...
You can also load your schema from code files like the below;
...
<Callout type="warning">
  **NOTE:** If you are using typescript and [path aliases](https://www.typescriptlang.org/tsconfig#paths), you may also
  need [tsconfig-paths](https://www.npmjs.com/package/tsconfig-paths). Further reading can be found at the [GitHub
...
<Callout type="info">This loader supports both browser and node environments.</Callout>
